...
class SecureTextServer:
    ...
    def reset_warning_timer(self, uid):
        ...
        if not sess:
            return
        if 'warning_timer' in sess and sess['warning_timer']:
    ...
    def send_warning(self, uid):
        sess = self.sessions.get(uid)
        if not sess or sess.get('warned'):
            return
        now = datetime.now()
        if now - sess['last_active'] < SESSION_TIMEOUT:
            conn = self.active_connections.get(uid)
            if conn:
                try:
                    conn.send(json.dumps({'type': 'WARNING', 'message': 'Your session will expire in 5 minutes'}).encode())
                except:
                    pass
            sess['warned'] = True
        ...
        while True:
            try:
                ...
                if uid:
                    ...
                    if sess:
                        if now - sess['last_active'] > SESSION_TIMEOUT:
                            if 'warning_timer' in sess: sess['warning_timer'].cancel()
                ...
                if cmd == 'CREATE_ACCOUNT':
                    ...
                    if uid:
                        sess = self.sessions.get(uid)
                        if sess and sess.get('warning_timer'):
                            sess['warning_timer'].cancel()
                        del self.active_connections[uid]
                        del self.sessions[uid]
                    ...
                    if not uid:
                        ...
                        if tgt in self.active_connections:
                            ...
                            try:
                                ...
                                logging.info("Relay payload: %s", pl)
                            except:
                                pass
...
class SecureTextClient:
    ...
    def _send_and_get_reply(self, obj):
        ...
        while time.time() < deadline:
            ...
            if msgs:
                self._process_messages(msgs)
                for m in msgs:
                    if 'status' in m and reply is None:
                        reply = m
            if reply:
                if reply.get('message') in ('Session expired', 'Action limit reached'):
    ...
    def reset_password(self):
        ...
        r = self._send_and_get_reply({
            'command': 'RESET_PASSWORD', 'target': tgt,
            'password': pw, 'totp': t, 'new_password': npw
        })
        print(r['message'])
