...
class SecureTextClient:
    ...
    def _recv_messages_blocking(self):
        ...
        while True:
            ...
            try:
                ...
                if split_idx==-1:
                    break
                first=text[:split_idx+1]
                rest=text[split_idx+1:]
                try:
                    obj=json.loads(first)
                    out.append(obj)
                    self.recv_buffer=rest.encode()
                except:
                    break
        return out
    ...
    def _send_and_get_reply(self,obj):
        ...
        while time.time()<deadline:
            ...
            for m in msgs:
                ...
                elif 'status' in m and reply is None:
                    reply=m
    ...
    def login(self):
        ...
        print(r['message'])
        if r['status']=='success':
            self.logged_in=True; self.username=u
    ...
    def send_message(self):
        ...
        r=self._send_and_get_reply({'command':'SEND_MESSAGE','recipient':t,'content':m})
        print(r['message'])
    ...
    def setup_e2ee(self):
        ...
        self.ecdh_private = private_key
        r=self._send_and_get_reply({'command':'REGISTER_PUBKEY','pubkey':public_key})
        print(r.get('message'))
        ...
        if r2.get('status')=='success':
            ...
            shared = self.ecdh_private.exchange(ec.ECDH(), peer_pub)
            derived = HKDF(
            ...
            self.session_key = derived
            print("Session key established")
        else:
            print(r2.get('message'))
