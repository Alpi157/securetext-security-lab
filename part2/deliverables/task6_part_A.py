...
class SecureTextClient:
    ...
    def get_challenge(self):
        resp = self.send_json({'command': 'GET_CHALLENGE'})
    ...
    def get_totp_challenge(self):
        resp = self.send_json({'command': 'GET_TOTP_CHALLENGE'})
        if resp.get('status') == 'success':
            return resp.get('challenge')
        return None
    ...
    def create_account(self):
        ...
        if resp['status']=='success':
            ...
            print("Save this secret for TOTP CR:", secret)

    def login(self):
        print("Choose challenge-response method:")
        print("1) Basic HMAC challenge")
        print("2) Time-based TOTP challenge")
        choice = input("> ").strip()
        if choice == '1':
            challenge = self.get_challenge()
            if not challenge or not self.respond_challenge(challenge):
                print("Basic CR failed")
                return
        elif choice == '2':
            username = input("Username for TOTP challenge: ").strip()
            c = self.get_totp_challenge()
            if c is None:
                print("Failed to get TOTP challenge")
                return
            code = input("Enter TOTP code: ").strip()
            if not self.respond_totp_challenge(username, code):
                return
        else:
        ...
        resp = self.send_json({'command':'LOGIN','username':u,'password':p,'totp':totp_code})
        print(resp['message'])
        if resp['status']=='success':
            self.logged_in = True
            self.username = u
